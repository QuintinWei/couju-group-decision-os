...
4. 默认使用「给我灵感」跨类型随机发现；若已有想法，再选择 1–6 个餐饮 / 娱乐倾向。
...
- 系统定位（需主动授权）与手输地铁站 / 商圈都会参与附近搜索和成员通勤估算；定位识别到六城之一时会自动同步城市，定位服务异常时保留有效坐标并提示手动输入；
...
- 高德地理编码把手输地铁站 / 商圈转换为坐标；浏览器定位会经坐标转换与逆地理编码，并在识别到六城时自动更新城市。两者都用于附近优先与通勤距离估算，不冒充实时公交或驾车路线；逆地理服务不可用时保留降低精度后的坐标。
