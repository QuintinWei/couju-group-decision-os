...
- 系统定位只在用户点击并授权后调用；坐标转换为附近区域标签并降低精度保存。
...
- 高德地理编码把手输地铁站 / 商圈转换为坐标；浏览器定位会经坐标转换与逆地理编码。两者都用于附近优先与通勤距离估算，不冒充实时公交或驾车路线。
