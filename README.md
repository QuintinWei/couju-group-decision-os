...
4. 默认使用「给我灵感」跨类型随机发现；候选先从全城召回，每位成员再选择 30 / 60 / 90 分钟或不限的单程通勤上限参与过滤。
5. 复制房间链接，用另一个浏览器或设备以朋友身份加入。
6. 每位成员先按 30 分钟粒度提交空闲时间，系统确定共同时间后再独立滑卡。
7. 如果整批没感觉，发起人可根据已划反馈换一批；设置预算、通勤和饮食 / 空间底线。
8. 至少两位真实成员完成后，启动公平求交集。
9. 查看唯一的「群体最优解」和每位成员满意度、公平度、Nash 福利、通勤与预算解释。
...
- 成员空闲时间按 30 分钟提交，服务端计算全员交集；无全员交集时不会伪造最终时间；
- 聚餐与活动两套完全独立的候选和结果链路；
...
通勤是硬约束里的一个例外。预算、时间、忌口都是用户直接声明的事实，超出即排除；而通勤时间目前是直线距离估算而不是路网时间，所以只有超过成员上限 15 分钟以上才排除，容差以内的候选保留但按超限幅度重罚，并在证据里明确标注是估算。这样估算误差不会静默删掉本来可行的地点。
...
- 高德地理编码把手输地铁站 / 商圈转换为坐标；浏览器定位会经坐标转换与逆地理编码，并在识别到十城时自动更新城市。候选从多个结果页召回并执行历史排除、同品牌去重和质量 / 探索分层，再按每位成员的通勤上限过滤。
