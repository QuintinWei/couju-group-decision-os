...
import { toPublicRoom } from "../lib/public-room.ts";
...
test("the public room DTO never serializes private cards, nominations, or private POI ids", () => {
  ...
  assert.equal(publicRoom.members[0].name, "成员");
  assert.equal(publicRoom.members[0].submittedAt, "2026-08-24T00:00:00.000Z");
  assert.equal(publicRoom.members[0].refreshRequestRound, 1);
  assert.doesNotMatch(serialized, /privateCandidates|nominatedCandidate|private-card-id|private-poi-id/);
});
