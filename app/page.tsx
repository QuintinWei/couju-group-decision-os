...
import type { JoinRoomDto, ParticipantMemberDto, ParticipantRoomDto, ParticipantSelfMember } from "../lib/public-room";
import type { RoundInsight } from "../lib/round-insight";
...
import { rejectionReasonOptions, sanitizeRejectionReasons, type RejectionReasonCode, type RejectionReasonRecord } from "../lib/rejection-feedback";
...
function roomNominationCount(room: ParticipantRoomDto) {
  return room.nominationCount;
}

function isParticipantSelfMember(member: ParticipantMemberDto | null | undefined): member is ParticipantSelfMember {
  return Boolean(member && "choices" in member);
}
...
export default function Home() {
  ...
  const [room, setRoom] = useState<ParticipantRoomDto | null>(null);
  ...
  const ranked = useMemo(() => (room?.decision?.rankings ?? []).filter((candidate) => !excludedIds.includes(candidate.id)), [room?.decision, excludedIds, appliedVetoReason]);
  const mainResult = ranked[0] ?? null;
  const participantMember = room?.members.find((member) => member.id === identity?.id) ?? null;
  const currentMember = isParticipantSelfMember(participantMember) ? participantMember : null;
  ...
  const refreshRoom = useCallback(async (code: string, quiet = false, auth?: MemberIdentity | null) => {
    ...
    try {
      ...
      const payload = await response.json() as { room?: ParticipantRoomDto; error?: string };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setIdentity(saved);
      if (saved) {
        void refreshRoom(code, false, saved).then((loaded) => { const member = loaded?.members.find((item) => item.id === saved.id); if (member) setStage(!member.availabilitySubmitted ? "availability" : "room"); else if (loaded) setStage("join"); });
      } else {
  ...
  const advanceCard = (nextChoices = swipes, nextReasons = rejectionReasons) => {
    if (cardIndex === candidates.length - 1) void submitCompletedChoices(nextChoices, nextReasons);
  ...
  const chooseCard = (choice: Choice) => {
    ...
    if (!current) return;
    const nextChoices = { ...swipes, [current.id]: choice };
    setSwipes(nextChoices);
    if (choice === "no") setPendingRejectionId(current.id);
    else advanceCard(nextChoices, rejectionReasons);
  };

  const finishRejection = (code: RejectionReasonCode | null, detail = "") => {
    const nextReasons = pendingRejectionId && code
      ? { ...rejectionReasons, [pendingRejectionId]: { code, ...(detail.trim() ? { detail: detail.trim().slice(0, 120) } : {}) } }
      : rejectionReasons;
    setRejectionReasons(nextReasons);
    setPendingRejectionId(null);
    advanceCard(swipes, nextReasons);
  };

  const saveCurrentChoices = async (choiceSnapshot = swipes, reasonSnapshot = rejectionReasons) => {
    if (!identity || !roomCode) throw new Error("成员身份已失效，请重新加入");
    const validRejectionReasons = sanitizeRejectionReasons(reasonSnapshot, candidates.map((candidate) => candidate.id), choiceSnapshot);
    const response = await fetch("/api/members", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ roomCode, memberId: identity.id, token: identity.token, expectedRound: room?.currentRound, budgetLabel: budget, commuteLabel: commute, setting, note: "", extraction: null, choices: choiceSnapshot, rejectionReasons: validRejectionReasons }),
    });
  ...
  const submitCompletedChoices = async (choiceSnapshot: Record<string, Choice>, reasonSnapshot: RejectionReasonRecord) => {
    setSyncing(true); setRoomError("");
    try {
      await saveCurrentChoices(choiceSnapshot, reasonSnapshot);
      setStage("room"); setToast("本轮选择已提交");
    } catch (error) {
      setRoomError(error instanceof Error ? error.message : "提交失败，请重试"); setStage("room");
    } finally {
      setSyncing(false);
    }
  };
  ...
  const loadRoundInsight = async () => {
    if (!identity || !roomCode) return null;
    try {
      const response = await fetch("/api/insights", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ roomCode, memberId: identity.id, token: identity.token }) });
      const payload = await response.json() as { insight?: RoundInsight };
      if (response.ok && payload.insight) { setRoundInsight(payload.insight); return payload.insight; }
    } catch { /* The deterministic conflict remains visible if DeepSeek is unavailable. */ }
    return null;
  };

  const openAdvanceSummary = async () => {
    await loadRoundInsight();
  ...
  const startRanking = () => {
    ...
    const people = readyMembers.map(() => ({ budget: "不限", commute: "不限" }));
  ...
  return <main className={`app ${stage === "home" ? "home-mode" : "demo-mode"}`}>
    <header className="app-header"><button className="brand couju-logo" onClick={() => stage !== "home" && resetSession()} aria-label="返回凑局首页"><span className="couju-logo-mark" aria-hidden="true"><i /><i /><i /></span><span className="couju-wordmark"><b>凑局</b><small>COUJU</small></span></button>{stage === "home" ? <span className="privacy-pill"><i /> 十城地点已上线</span> : <div className="demo-header-right"><div className="step-dots" aria-label={`Demo 进度 ${currentProgress + 1}/${stageOrder.length}`}>{stageOrder.map((item, index) => <span key={item} className={index <= currentProgress ? "active" : ""} />)}</div><span className={`demo-badge mode-${candidateMeta.mode}`}>{candidateMeta.mode === "live" ? "地点推荐" : "演示数据"}</span><button className="quiet-button" onClick={resetSession}>退出</button></div>}</header>
    {stage === "home" && <HomeScreen onStart={(kind) => { updateConfig({ ...configRef.current, kind }); setStage("create"); }} />}
    ...
    {stage === "room" && room && <RoomScreen room={room} currentMember={currentMember} syncing={syncing} error={roomError} onShare={copyShare} onPreference={() => { setCardIndex(0); setSwipes(currentMember?.choices ?? {}); setRejectionReasons(currentMember?.rejectionReasons ?? {}); setBudget(currentMember?.budgetLabel || budget); setCommute(currentMember?.commuteLabel || commute); setSetting(currentMember?.setting || setting); setNote(currentMember?.note || ""); setExtraction(currentMember?.extraction ?? null); setPrivateCandidates([]); setPrivateNominationId(null); setPrivateError(""); setStage(getPreferenceEntryStage({ currentRound: room.currentRound, constraintsReady: Boolean(currentMember?.constraintsReady), groupIntersection: room.meta.groupIntersection })); }} onRank={startRanking} onRequestNextRound={requestNextRound} onOpenAdvance={openAdvanceSummary} />}
    {stage === "setup" && <PreferenceSetupScreen config={config} budget={budget} commute={commute} setting={setting} setBudget={setBudget} setCommute={setCommute} setSetting={setSetting} onBack={() => setStage("room")} onContinue={saveStartingConstraints} loading={syncing} error={roomError} />}
    ...
    {advanceConfirmOpen && room && <div className="modal-backdrop"><button className="modal-dismiss-layer" onClick={() => !advanceLoading && setAdvanceConfirmOpen(false)} aria-label="关闭开启下一轮确认" /><section className="advance-modal" role="dialog" aria-modal="true" aria-labelledby="advance-title"><button className="modal-close" onClick={() => !advanceLoading && setAdvanceConfirmOpen(false)} aria-label="关闭">×</button><span className="modal-icon">↻</span><h2 id="advance-title">确认开启下一轮？</h2><div className="ai-learning-summary"><b>AI 学到了什么</b><p>{roundInsight?.learned || "系统会根据全体喜欢、拒绝及原因调整下一轮候选。"}</p>{roundInsight?.nextRound && <small>{roundInsight.nextRound}</small>}</div><div className="advance-modal-note"><b>下一轮 12 张卡</b><span>{roomNominationCount(room)} 张成员提名 · {8 - Math.min(8, roomNominationCount(room))} 张反馈学习 · 4 张新类型探索</span></div><button className="full-dark-button" onClick={advanceRound} disabled={advanceLoading}>{advanceLoading ? "正在生成下一轮…" : "汇总提名与反馈，开启下一轮"} <span>→</span></button></section></div>}
    {toast && <div className="toast" role="status"><span>✓</span>{toast}</div>}
  </main>;
}

function HomeScreen({ onStart }: { onStart: (kind: DecisionKind) => void }) {
  return <section className="home-landing"><div className="home-spark spark-left">✦</div><div className="home-spark spark-right">↗</div><div className="home-intro"><h1>不知道干啥？<em>别再纠结了</em> <span>👋</span></h1><p>凑局不是随便推荐，而是帮一群人找到都能接受的方案。</p></div><div className="home-choice-grid"><article className="home-choice-card food-choice"><div className="choice-heading"><span className="choice-icon">🍴</span><div><h2>一起吃饭</h2><p>今天吃什么好呢？</p></div></div><img src="/home/food-hero.png" alt="拉面、汉堡、果汁和披萨的三维插画" /><button onClick={() => onStart("dining")}>开始吃饭局 <span>→</span></button></article><article className="home-choice-card activity-choice"><div className="choice-heading"><span className="choice-icon">◆</span><div><h2>出去玩</h2><p>去哪玩才好玩？</p></div></div><img src="/home/activity-hero.png" alt="帐篷、相机和背包的三维插画" /><button onClick={() => onStart("activity")}>开始玩乐局 <span>→</span></button></article></div><div className="home-process"><div><i>01</i><span><b>创建或加入</b><small>创建新局或加入好友的局</small></span></div><em>→</em><div><i>02</i><span><b>邀请好友</b><small>喊上好友一起参与</small></span></div><em>→</em><div><i>03</i><span><b>一起决策</b><small>每轮选择，收集大家意见</small></span></div><em>→</em><div><i>04</i><span><b>见证结果</b><small>达成共识，开始出发</small></span></div></div><p className="home-privacy">🔒 隐私安全保护　|　所有偏好仅用于计算，不对外公开</p></section>;
}
...
function CreateScreen(props: { config: RoomConfig; creatorName: string; creatorOrigin: string; creatorLocation: GeoPoint | null; locationStatus: string; locating: boolean; ideaMode: boolean; selectedInterests: string[]; avoid: string; setCreatorName: (value: string) => void; setCreatorOrigin: (value: string) => void; setIdeaMode: (value: boolean) => void; setSelectedInterests: (value: string[]) => void; setAvoid: (value: string) => void; onLocate: () => void; error: string; onChange: (config: RoomConfig) => void; onBack: () => void; onCreate: () => void; loading: boolean }) {
  ...
  return <section className={`flow-page create-page create-${props.config.kind}`}>
    <button className="back-button" onClick={props.onBack}>← 返回</button>
    <ScreenTitle eyebrow={props.config.kind === "dining" ? "CREATE A FOOD ROOM" : "CREATE AN ACTIVITY ROOM"} title={`创建一个${props.config.kind === "dining" ? "吃饭" : "玩乐"}局 👋`} detail="填好日期和地点，剩下的交给大家一起选。" />
    <div className="create-layout single-card"><div className="form-card">
      ...
      {(error || props.error) && <p className="form-error" role="alert">{error || props.error}</p>}<button className="full-dark-button" onClick={submit} disabled={props.loading}>{props.loading ? "正在准备候选…" : `创建${props.config.kind === "dining" ? "吃饭" : "玩乐"}局`} <span>→</span></button>
    </div></div>
  </section>;
}
...
function RoomScreen({ room, currentMember, syncing, error, onShare, onPreference, onRank, onRequestNextRound, onOpenAdvance }: { room: ParticipantRoomDto; currentMember: ParticipantSelfMember | null; syncing: boolean; error: string; onShare: () => void; onPreference: () => void; onRank: () => void; onRequestNextRound: () => void; onOpenAdvance: () => void }) {
  const { config, meta } = room;
  const availabilityCount = room.members.filter((member) => member.availabilitySubmitted).length;
...
function PrivateDiscoveryScreen({ config, cards, selectedId, member, loading, error, onSelect, onSubmit, onSkip, onBack }: { config: RoomConfig; cards: Candidate[]; selectedId: string | null; member: ParticipantSelfMember; loading: boolean; error: string; onSelect: (id: string | null) => void; onSubmit: () => void; onSkip: () => void; onBack: () => void }) {
  return <section className="flow-page private-discovery-page"><button className="back-button" onClick={onBack} disabled={loading}>← 返回本轮结果</button><ScreenTitle eyebrow="PRIVATE DISCOVERY" title="再给你三张，只由你决定" detail="这不是最终结果：最多提名一张，下一轮会交给所有成员共同评价。" /><div className="private-notice" role="status"><span>🔒</span><b>仅你可见 · 提名后进入下一轮共享评选</b><small>不会在当前房间候选或同步数据中展示</small></div><div className="private-card-grid">{cards.map((card) => {
...
function RankingScreen({ config, step, candidates, ranked, meta }: { config: RoomConfig; step: number; candidates: Candidate[]; ranked: RankedCandidate[]; meta: CandidateMeta }) {
  ...
  const rows = [{ n: candidates.length, title: "地点候选已加载", detail: `${config.city} · ${meta.label}` }, { n: ranked.length, title: "共同底线过滤完成", detail: "任一成员明确拒绝即排除" }, { n: memberCount, title: "真实成员效用已计算", detail: "滑卡、预算和通勤全部进入计算" }, { n: ranked[0]?.groupFit ?? 0, title: "公平排序完成", detail: "满意度门槛 · Pareto · Nash 福利" }];
...
function ResultsScreen({ config, room, currentMember, ranked, meta, members, aiExplanation, roundInsight, error, advancing, privateLoading, onPrivateDiscovery, onAcceptCommute, onVeto, onLock, onAdjust, onDiscuss, onRequestNextRound, onOpenAdvance }: { config: RoomConfig; room: ParticipantRoomDto; currentMember: ParticipantSelfMember | null; ranked: RankedCandidate[]; meta: CandidateMeta; members: ParticipantMemberDto[]; aiExplanation: AiExplanation | null; roundInsight: RoundInsight | null; error: string; advancing: boolean; privateLoading: boolean; onPrivateDiscovery: () => void; onAcceptCommute: (minutes: number) => void; onVeto: (selected: RankedCandidate) => void; onLock: (selected: RankedCandidate) => void; onAdjust: () => void; onDiscuss: () => void; onRequestNextRound: () => void; onOpenAdvance: () => void }) {
  const main = ranked[0];
