...
function roomNominationCount(room: StoredRoom) {
  const published = (room as StoredRoom & { nominationCount?: number }).nominationCount;
  return Number.isInteger(published) ? published! : room.members.reduce((count, member) => count + (member.nominatedCandidate ? 1 : 0), 0);
}
...
function CreateScreen(props: { config: RoomConfig; creatorName: string; creatorOrigin: string; creatorLocation: GeoPoint | null; locationStatus: string; locating: boolean; ideaMode: boolean; selectedInterests: string[]; avoid: string; setCreatorName: (value: string) => void; setCreatorOrigin: (value: string) => void; setIdeaMode: (value: boolean) => void; setSelectedInterests: (value: string[]) => void; setAvoid: (value: string) => void; onLocate: () => void; error: string; onChange: (config: RoomConfig) => void; onBack: () => void; onCreate: () => void; loading: boolean }) {
  ...
  return <section className={`flow-page create-page create-${props.config.kind}`}>
    ...
    </div></div><div className="create-process"><div><i>1</i><b>创建或加入</b><small>创建新局或加入好友的局</small></div><span>→</span><div><i>2</i><b>邀请好友</b><small>喊上好友一起参与</small></div><span>→</span><div><i>3</i><b>一起决策</b><small>每轮选择，收集大家意见</small></div><span>→</span><div><i>4</i><b>见证结果</b><small>达成共识，开始出发</small></div></div>
  </section>;
}
...
function ResultsScreen({ config, room, currentMember, ranked, meta, members, aiExplanation, roundInsight, error, advancing, privateLoading, onPrivateDiscovery, onAcceptCommute, onVeto, onLock, onAdjust, onDiscuss, onRequestNextRound, onOpenAdvance }: { config: RoomConfig; room: StoredRoom; currentMember: StoredMember | null; ranked: RankedCandidate[]; meta: CandidateMeta; members: StoredMember[]; aiExplanation: AiExplanation | null; roundInsight: RoundInsight | null; error: string; advancing: boolean; privateLoading: boolean; onPrivateDiscovery: () => void; onAcceptCommute: (minutes: number) => void; onVeto: (selected: RankedCandidate) => void; onLock: (selected: RankedCandidate) => void; onAdjust: () => void; onDiscuss: () => void; onRequestNextRound: () => void; onOpenAdvance: () => void }) {
  ...
  const canAdvance = controls.canAdvance;
  if (!main) {
    const reasons = diagnoseRoundConflict(room.candidates, room.members, config).slice(0, 2);
