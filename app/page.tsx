...
export default function Home() {
  ...
  return <main className={`app ${stage === "home" ? "home-mode" : "demo-mode"}`}>
    <header className="app-header"><button className="brand" onClick={() => stage !== "home" && resetSession()} aria-label="返回凑局首页"><span className="brand-mark">凑</span><span>凑局</span><small>COUJU</small></button>{stage === "home" ? <span className="privacy-pill"><i /> 上海首发 · 六城保留</span> : <div className="demo-header-right"><div className="step-dots" aria-label={`Demo 进度 ${currentProgress + 1}/7`}>{stageOrder.map((item, index) => <span key={item} className={index <= currentProgress ? "active" : ""} />)}</div><span className={`demo-badge mode-${candidateMeta.mode}`}>{candidateMeta.mode === "live" ? "高德地点" : "演示数据"}</span><button className="quiet-button" onClick={resetSession}>退出</button></div>}</header>
...
function HomeScreen({ onStart }: { onStart: () => void }) {
  return <><section className="hero"><div className="hero-copy"><div className="eyebrow"><i /> EVIDENCE-BASED GROUP DECISION</div><h1>不是猜一个答案，<br /><em>是算出交集。</em></h1><p>真实输入会改变推荐。地点记录来源，DeepSeek 只抽取字段，确定性算法负责过滤与公平排序。</p><div className="hero-actions"><button className="primary-button" onClick={onStart}>从上海开始体验 <span>→</span></button><span><b>六城可选</b> · 数据模式全程可见</span></div><div className="hero-proof"><div className="proof-faces"><i>数</i><i>AI</i><i>算</i></div><span><b>地点、抽取、排序三层分开</b><small>不再把模拟结果包装成实时推荐</small></span></div></div><div className="decision-card" aria-label="凑局推荐证据预览"><div className="floating-chat chat-one"><b>人均 150</b><small>明确进入预算过滤</small></div><div className="floating-chat chat-two"><b>别太辣</b><small>DeepSeek → 待确认标签</small></div><div className="card-topline"><span><i /> 上海聚餐 · 示例</span><b>TRACEABLE</b></div><div className="trust-stack"><div><span>01</span><p><b>地点来源</b><small>高德 POI / 演示数据明确区分</small></p><em>可追溯</em></div><div><span>02</span><p><b>字段抽取</b><small>DeepSeek V4 Flash / 规则降级</small></p><em>可确认</em></div><div><span>03</span><p><b>公平排序</b><small>预算、通勤、滑卡真正参与</small></p><em>可复算</em></div></div><div className="place-result"><div className="result-visual photo-preview"><img src="/candidates/food-yunnan.jpg" alt="聚餐候选示例" /><span>EXAMPLE</span></div><div><small>结果会随输入变化</small><h3>Group Fit 动态计算</h3><p>最低效用 · 群体均值 · 不确定性</p></div><strong>ƒ(x)<small>FAIRMIX</small></strong></div><div className="fit-line"><i>✓</i><span><b>证据不足会显示“待确认”</b><small>不再声称实时可订或全部核验</small></span></div></div></section><section className="how-strip"><div><b>01</b><span>加载带来源候选</span><small>上海首发，其他城市共用接口</small></div><div className="arrow">→</div><div><b>02</b><span>抽取并确认底线</span><small>DeepSeek Key 缺失时明确降级</small></div><div className="arrow">→</div><div><b>03</b><span>确定性公平排序</span><small>每条推荐都有计算依据</small></div></section></>;
}
...
function RoomScreen({ config, completed, meta, candidateCount, onShare, onPreference, onRank }: { config: RoomConfig; completed: boolean; meta: CandidateMeta; candidateCount: number; onShare: () => void; onPreference: () => void; onRank: () => void }) {
  ...
  return <section className="flow-page room-page"><div className="room-kicker"><span>{meta.mode === "live" ? "真实地点模式" : "演示候选模式"}</span><b>房间 CJ-{config.date.slice(5).replace("-", "")}</b></div><ScreenTitle eyebrow={`${config.kind === "dining" ? "DINNER" : "WEEKEND"} IN ${config.city.toUpperCase()}`} title={roomTitle(config.kind)} detail={`${formatDate(config.date)} · ${config.startTime}–${config.endTime} · ${config.city}`} /><div className="data-audit-strip"><span className={`source-dot ${meta.mode}`} /><b>{meta.label}</b><span>{candidateCount} 个候选</span><span>更新 {formatSourceTime(meta.fetchedAt)}</span><i>{meta.disclaimer}</i></div><div className="room-grid"><div className="room-main-card"><div className="room-card-head"><div><span>本轮参与信号</span><strong>{doneCount}/{config.people} 已准备</strong></div><i>{completed ? "可以运行 FairMix" : "等待你的真实输入"}</i></div><div className="member-list">{people.map((person) => <div key={person.name} className={person.done ? "member done" : "member pending"}><div className="avatar">{person.letter}{person.done && <span>✓</span>}</div><div><b>{person.name}</b><small>{person.virtual ? "演示成员样本" : person.done ? "你的偏好已提交" : "待完成"}</small></div><em>{person.virtual ? "虚拟" : person.done ? "完成" : "待完成"}</em></div>)}</div>{!completed ? <button className="full-dark-button pulse" onClick={onPreference}>完成我的偏好 <span>→</span></button> : <button className="full-dark-button lime-button" onClick={onRank}>开始确定性求交集 <span>✦</span></button>}<p className="privacy-note">⌾ 当前版本仅在本次浏览器会话中处理你的输入；实时多人房间尚未启用</p></div><aside className="invite-card transparency-card"><span className={`big-source-mark ${meta.mode}`}>{meta.mode === "live" ? "高" : "DEMO"}</span><h3>{meta.mode === "live" ? "地点事实来自高德" : "当前使用演示候选"}</h3><p>{meta.mode === "live" ? "记录 POI ID、地址、坐标与返回时间；价格和营业仍需确认。" : "用于稳定展示产品流程，不代表现实商户或可订状态。"}</p><button onClick={onShare}>复制 Demo 链接 <span>↗</span></button><small>推荐分仍由当前输入实时计算</small></aside></div><div className="public-constraint"><b>公开配置</b><span>{formatDate(config.date)} {config.startTime}–{config.endTime}</span><span>{config.city}市</span><span>{config.people} 人</span><i>{config.people - 1} 位虚拟成员会明确参与演示计算</i></div></section>;
}
