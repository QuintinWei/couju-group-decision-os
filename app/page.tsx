...
const stageOrder: Stage[] = ["create", "room", ...PREFERENCE_FLOW, "ranking", "results", "locked"];
...
export default function Home() {
  ...
  const refreshCandidates = async () => {
    ...
    try {
      ...
      const payload = await loadCandidates({ location: center, strategy: "learn", interests: liked.slice(0, 3), avoid: [...(candidateMeta.avoid || []), ...dislikedNames].slice(-8), exclude: candidates.map((candidate) => candidate.id), page: ((candidateMeta.page || 1) % 5) + 1 });
      ...
      await refreshRoom(roomCode, true); setStage("room"); setToast(liked.length ? "已参考本轮反馈，房间已进入新一轮选择" : "已换成另一组跨类型候选，房间已进入新一轮选择");
...
function RoomScreen({ room, currentMember, syncing, isCreator: creator, refreshing, error, onRefresh, onShare, onPreference, onRank }: { room: StoredRoom; currentMember: StoredMember | null; syncing: boolean; isCreator: boolean; refreshing: boolean; error: string; onRefresh: () => void; onShare: () => void; onPreference: () => void; onRank: () => void }) {
  ...
  const isCreator = canRefreshCandidates(creator, "room", false);
...
function PreferenceSetupScreen(props: { config: RoomConfig; budget: string; commute: string; setting: string; setBudget: (value: string) => void; setCommute: (value: string) => void; setSetting: (value: string) => void; onBack: () => void; onContinue: () => void }) {
  ...
  return <section className="flow-page constraints-page"><button className="back-button" onClick={props.onBack}>← 返回房间</button><ScreenTitle eyebrow="YOUR STARTING POINT" title="先设定你的选择边界" detail="预算和通勤先排除明显不合适的选择；场景偏好只调整排序，不会锁死推荐类型。" /><div className="constraint-layout preference-setup-layout"><div className="constraint-card"><div className="constraint-label"><span>01</span><div><b>人均预算</b><small>超过上限的已知价格候选不会进入最终结果</small></div></div><ChipGroup label="人均预算" values={dining ? ["≤ ¥100", "≤ ¥150", "≤ ¥200", "不限"] : ["≤ ¥100", "≤ ¥200", "≤ ¥300", "不限"]} selected={props.budget} onSelect={props.setBudget} /><div className="constraint-label"><span>02</span><div><b>最远单程通勤</b><small>从你填写或定位的出发地到候选地点估算</small></div></div><ChipGroup label="最远单程通勤" values={["≤ 30 分钟", "≤ 60 分钟", "≤ 1.5 小时", "不限"]} selected={props.commute} onSelect={props.setCommute} /><div className="constraint-label"><span>03</span><div><b>{dining ? "用餐氛围" : "场景偏好"}</b><small>这是软偏好，只影响卡片顺序和最终得分</small></div></div><ChipGroup label={dining ? "用餐氛围" : "场景偏好"} values={dining ? ["安静聊天", "热闹聚会", "都可以"] : ["室内优先", "户外优先", "都可以"]} selected={props.setting} onSelect={props.setSetting} /></div></div><button className="confirm-preference" onClick={props.onContinue}>按这些条件开始选卡片 <span>→</span></button></section>;
}

function SwipeScreen({ config, cards, index, choices, travelMinutes, canRefresh, refreshing, onRefresh, onChoose, onBack, onPointerDown, onPointerUp }: { config: RoomConfig; cards: Candidate[]; index: number; choices: Record<string, Choice>; travelMinutes: number | null; canRefresh: boolean; refreshing: boolean; onRefresh: () => void; onChoose: (choice: Choice) => void; onBack: () => void; onPointerDown: (x: number) => void; onPointerUp: (x: number) => void }) {
...
function PreferenceDetailsScreen(props: { config: RoomConfig; note: string; extraction: PreferenceExtraction | null; loading: boolean; submitting: boolean; error: string; setNote: (value: string) => void; onParse: () => void; onRemoveSignal: (id: string) => void; onBack: () => void; onConfirm: () => void }) {
