...
export default function Home() {
  ...
  return <main className={`app ${stage === "home" ? "home-mode" : "demo-mode"}`}>
    ...
    {stage === "swipe" && <SwipeScreen config={config} cards={candidates} index={cardIndex} choices={swipes} canRefresh={canRefreshCandidates(room?.members[0]?.id === identity?.id, stage, ranked.length > 0)} refreshing={candidateRefreshing} onRefresh={refreshCandidates} onChoose={chooseCard} onBack={() => setStage("room")} onPointerDown={(x) => { pointerStart.current = x; }} onPointerUp={(x) => { if (pointerStart.current === null) return; const delta = x - pointerStart.current; if (delta > 65) chooseCard("like"); else if (delta < -65) chooseCard("no"); pointerStart.current = null; }} />}
