...
export async function POST(request: Request) {
  ...
  try { body = await request.json() as ExplainInput; }
  ...
  const members = Array.isArray(body.members) ? body.members.slice(0, 6) : [];
    ...
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `请用简洁中文解释第一名为什么是群体平衡解。输出 {"headline":"20字内","reasoning":"70字内","tradeoff":"40字内"}。城市：${String(body.city || "")}; 场景：${String(body.kind || "")}; 成员：${JSON.stringify(members)}; 已排序候选：${JSON.stringify(candidates)}` },
        ],
