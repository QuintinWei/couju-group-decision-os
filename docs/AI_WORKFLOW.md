...
通勤是唯一按容差处理的约束。预算、时间、忌口来自用户直接声明，超出即 `H_i(c) = 0`；通勤时间当前是直线距离估算而非路网时间，因此只有超过成员上限 15 分钟以上才置 0，容差以内保留候选并在个人效用里按超限幅度重罚（超限时效用因子降到 0.5 以下，随超限比例继续下降）。接入实时路线矩阵后这条容差应当收窄。
...
最终扣除 0.35 × Uncertainty，并优先保留 Pareto 前沿候选。
```

权重设计优先保护最不满意的人，同时避免一个极端低分被整体均值掩盖。Uncertainty 上限为 0.32，因此演示数据或缺少可核验事实的候选最多被扣掉约 11 分，足以让事实完整的候选在同分区间胜出。
...
活动检索使用头疗按摩、攀岩、电影、陶艺泥塑、KTV、拼豆手作、剧本杀、麻将棋牌、桌游、密室、球类、演出和景点等关键词分桶，再按桶轮流取样保持类别多样性，不额外按 POI 名称过滤景点。候选先从全城召回，最终由各成员选择的 30 / 60 / 90 分钟或不限通勤上限做硬过滤。
...
| 调用方不是有效房间成员 | 直接拒绝，不消耗 DeepSeek 额度；无 Key 的规则抽取仍然可用 |
| 输出不符合 Schema | 丢弃输出并要求用户手动确认，不静默猜测 |
| 地点 API 不可用 | 使用带更新时间、明确标注为演示数据的本地候选，不声称实时可订 |
| 高德日配额压力 | 按城市、类型、关键词、页码缓存上游召回 6 小时，缓存值保留真实抓取时间 |
